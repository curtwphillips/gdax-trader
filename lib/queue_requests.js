...
exports.getInfo = function () {
  ...
  if (q.length > config.thresholds.maxQueue) {
    ...
    if (utility.getMsBetween(lastAlert, now) > config.delays.resendAlert) {
      log.error(
        q.length +
          " jobs are queued. Check whether a problem, such as bitcoind not responding, is stopping jobs from processing."
      );
      lastAlert = now;
    }
  }
...
function setupJob(job) {
  try {
    ...
    log.info(
      "job id " + job.id + " failed with opts " + JSON.stringify(job.opts)
    );
    log.info(err.stack);
    exports.info.errors++;
    throw err;

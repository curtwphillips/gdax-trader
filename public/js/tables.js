let tables = (function () {
  ...
  let lastTable; // tracks the most recently setup table
  let found = false;
  let madeTables = {};
  // headers added by us
  let bpTagsHeader = "BP Tags";
  let bpNotesHeader = "BP Notes";
  let startStopHeader = "Start/Stop";
  ...
  function create(table) {
    try {
      ...
      if (table.searchButtonId) {
        document.getElementById(table.searchButtonId).innerText =
          "Rendering...";
      }
  ...
  function setHeaders(table) {
    try {
      let columnOrder = table.columnOrder;
      let headers = [];
      let rows = table.rows; // { pristineRow: [...] }
      let i, j, header, row;
      // Go through each row and save keys as headers
      for (i = 0; i < rows.length; i++) {
        row = rows[i];
        for (let key in row.pristineRow) {
          if (
            headers.indexOf(key) === -1 &&
            (!table.removeColumns || table.removeColumns.indexOf(key) === -1)
          ) {
            headers.push(key);
          }
        }
      ...
      for (i = 0; i < rows.length; i++) {
        ...
        for (j = 0; j < headers.length; j++) {
          ...
          // convert values
          if (
            row.pristineRow[header] &&
            table.convertColumns &&
            table.convertColumns[header]
          ) {
            row.pristineRow[header] = utility[table.convertColumns[header]](
              row.pristineRow[header]
            );
          } else {
    ...
    try {
      ...
      for (let i = 0; i < table.rows.length; i++) {
        if (table.rows[i].checked) {
          document
            .getElementById(table.title + "-checkbox-" + i)
  ...
  function setRow(table, i) {
    try {
      let tagIncludeKeys = ["Name", "Description"];
      let j, k, row, header;
      ...
      if (
        row.pristineRow &&
        row.pristineRow.InstanceId &&
        row.pristineRow.InstanceId.length > 3
      ) {
        hasInstanceId = true;
      }
      ...
      if (row.pristineRow.State && table.allowStartStop && hasInstanceId) {
        ...
        if (row.allowStartStop) {
          row.tableRow[startStopHeader] =
            row.pristineRow.State.Name === "running" ? "Stop" : "Start";
      ...
      for (j = 0; j < headers.length; j++) {
        ...
        if (header === "Tags") {
          ...
          for (k = 0; k < tagIncludeKeys.length; k++) {
            ...
            tagIncludeKeysFound = _.findWhere(row.pristineRow.Tags, {
              Key: tagIncludeKeys[k],
            });
          ...
          if (table.dayColumns && table.dayColumns.indexOf(header) !== -1) {
            // convertOn = false;
            row.tableRow[header] = row.pristineRow[header].map(function (
              dayNum
            ) {
          ...
          if (table.combineColumns && table.combineColumns[header]) {
            ...
            for (k = 0; k < table.combineColumns[header].length; k++) {
              if (row.pristineRow[table.combineColumns[header][k]]) {
                row.tableRow[header].push(
                  row.pristineRow[table.combineColumns[header][k]]
                );
              }
            }
          }
          ...
          if (
            !table.skipFlattening ||
            table.skipFlattening.indexOf(header) === -1
          ) {
          ...
          if (
            ...
            table.numericColumns.indexOf(header) > -1 &&
            table.useRoundNumbers
          ) {
          ...
          if (
            header !== bpTagsHeader &&
            header !== bpNotesHeader &&
            typeof row.tableRow[header] === "string"
          ) {
            if (!table.noReplaceChars) {
              row.tableRow[header] = replaceChars(
                row.tableRow[header],
                table.errorMsgId
              );
            }
          ...
          if (
            table.formatDateColumns &&
            table.formatDateColumns.indexOf(header) !== -1
          ) {
            row.tableRow[header] = utility.getTime(
              new Date(row.tableRow[header]),
              true
            );
          }
        }
      ...
      if (table.tags) {
        let tagsColumn = bpTagsHeader;
        row.tableRow[tagsColumn] = [];
        for (j = 0; j < table.tags.length; j++) {
          let tag = table.tags[j];
          for (k = 0; k < tag.usedBy.length; k++) {
            let usedBy = tag.usedBy[k];
            if (
              usedBy.kind === table.title &&
              usedBy.identifier === row.pristineRow[table.identifier]
            ) {
      ...
      if (table.notes) {
        let notesColumn = bpNotesHeader;
        row.tableRow[notesColumn] = [];
        let associationColumn = table.identifier;
        for (j = 0; j < table.notes.length; j++) {
          let note = table.notes[j];
          if (
  ...
  function setRows(table) {
    try {
      ...
      if (table.rows[0].allowStartStop) {
        // add the Start/Stop header
  ...
  function formatTimeColumns(timeData) {
    try {
      ...
      let minute = timeData.minute.toString();
      if (minute.length === 1) {
        minute = "0" + minute;
      }
  ...
  function getNewRowValue(table, row, header) {
    try {
      ...
      let columnData = row.tableRow[header]
        ? row.tableRow[header]
        : row.pristineRow[header];
    ...
    try {
      ...
      if (Array.isArray(obj)) {
        ...
        for (let key in obj) {
          if (
            keyConversions &&
            (keyConversions[key] || keyConversions[key] === "")
          ) {
  ...
  function addTagsNotesColumns(table) {
    try {
      ...
      if (
        !table.showTagsNotes ||
        (!table.showTagsNotes.tags && !table.showTagsNotes.notes)
      ) {
        return;
      }
      // rows to check
      ...
      let reqs = table.showTagsNotesRequirements;
      if (reqs) {
        let show = true;
        for (let key in table.showTagsNotesRequirements) {
  ...
  function exportToCSV(csvTitle) {
    try {
      ...
      let filename = "Portal-" + csvTitle.replace(/ /g, "") + ".csv"; //gen a filename using the csvTitle without spaces
  ...
  function sortRebuild(table, colNum) {
    try {
      let i, j, k, l, index, moveTo;
      let dataLen = table.rows.length;
      // sort the rows into the expected order
      if (colNum === table.lastSortColumn) {
        // reversing last search
  ...
  function sortBy(table, colNum) {
    try {
      ...
      let headerElem = $("#" + table.title + "-headers-" + colNum + " div");
  ...
  function merge(left, right, col, isNumeric) {
    try {
      ...
      while (left.length > 0 && right.length > 0) {
        if (isNumeric) {
          if (
            Number(left[0].text[table.headers[col]].replace(",", "")) <=
            Number(right[0].text[table.headers[col]].replace(",", ""))
          ) {
          ...
          if (
            left[0].text[table.headers[col]] <=
            right[0].text[table.headers[col]]
          ) {
            result.push(left.shift());
          } else {
            result.push(right.shift());
  ...
  function addStartStopClickEvent(elem, table, i) {
    try {
      elem.addEventListener("click", function (event) {
        ...
        let startOrStop = $(event.target).text();
  ...
  function buildTable(table) {
    try {
      if (table.limitRowsInputId) {
        table.limitRowsAmount = document.getElementById(
          table.limitRowsInputId
        ).value;
      ...
      tableIsRendering = true;
      let i, j, k, elem, elem1, elem2, elem3, elem4, innerHTML, className;
      let title = table.title;
      table.rowCount = table.rows.length;
      table.unfilteredCount = table.rows.length;
      let tableContainer = document.getElementById(table.tableContainerId);
      ...
      for (i = 0; i < table.headers.length; i++) {
        ...
        let headerText = "";
        if (
          table.headerConversions &&
          table.headerConversions[table.headers[i]]
        ) {
      ...
      let divideInto = Math.ceil(rowCount / 4000);
      let chunkSize = Math.ceil(rowCount / divideInto);
      let iteration = 0;
      ...
      setTimeout(function buildRows() {
        ...
        for (i = 0; i < chunkSize; i++) {
          ...
          if (
            table.limitRowsAmount &&
            table.visibleRows >= table.limitRowsAmount
          ) {
            break;
          }
        ...
        if (iteration < divideInto) {
          ...
          if (table.noFilters) {
            ...
            captionElem.textContent =
              "Showing " +
              table.visibleRows.toLocaleString() +
              " of " +
          ...
          if (table.searchButtonId) {
            setTimeout(function () {
              ...
              document
                .getElementById(table.exportButtonId)
  ...
  function buildCol(table, colText) {
    try {
      if (utility.isString(table)) {
        // adding to pre-made table
        table = madeTables[table];
      }
      if (!table) {
        throw new Error(
          "table " + table + " not found when adding " + colText + " column"
        );
      }
      theadTrElem = document.getElementById(table.title + "-table-header-tr");
      if (!theadTrElem) {
        throw new Error(
          "header row not found when adding " + colText + " column"
        );
      }
      let colNum = table.headers.length;
      ...
      if (
        table.headerConversions &&
        table.headerConversions[table.headers[colNum]]
      ) {
  ...
  function buildRow(table, rowNum, rowData, tbodyElem, hostname) {
    try {
      ...
      if (rowData.filteredCells) {
        for (let j = 0; j < rowData.filteredCells.length; j++) {
      ...
      if (table.htmlProperties) {
        for (let i = 0; i < table.htmlProperties.length; i++) {
          let key = table.htmlProperties;
          let val;
      ...
      for (j = 0; j < table.headers.length; j++) {
        ...
        if (table.headers[j] === startStopHeader) {
          ...
          if (
            table.nowrapColumns &&
            table.nowrapColumns.indexOf(table.headers[j]) !== -1
          ) {
          ...
          elem3.textContent = table.rows[based_i].tableRow[bpTagsHeader].join(
            "<br />"
          );
          ...
          for (
            let k = 0;
            k < table.rows[based_i].tableRow[header].length;
            k++
          ) {
            innerHTML +=
              '<span class="nowrap">' +
              table.rows[based_i].tableRow[header][k] +
              "</span><br />";
          }
          ...
          table.headers[j] === "InstanceId" &&
          (title === "EC2s" || title === "NATs")
        ) {
          ...
          elem2.setAttribute(
            "title",
            table.rows[based_i].pristineRow.State.Name
          );
          // inner div to hold circle
          elem3 = document.createElement("div");
          elem3.id = "row-" + based_i + "-running-circle";
          if (
            table.rows[based_i].pristineRow.State.Name === "running" ||
            table.rows[based_i].pristineRow.State === "available"
          ) {
  ...
  function attachBPNotesViewElements(attachTo, data, i) {
    try {
      ...
      let topElement = $("#row-" + i + "-bpnotes-view");
  ...
  function enableStartStopTable(table) {
    try {
      let allowFinal = false;
      let reqs = table.startStopRequirements;
      if (!reqs) {
        return false;
      }
      if (reqs.userRole) {
  ...
  function enableStartStopRow(table, index) {
    try {
      ...
      // allowFinal letiable: set to true is used to bypass further checks
      let allowFinal = false;
  ...
  function filterBy(table, columnNumber) {
    try {
      ...
      let filterText = $(
        "[id='" + table.title + "-filter-" + columnNumber
      ).val();
      // set each filter value for the column
      if (filterText !== "") {
        // text in the filter
        let filterValue = filterText.toLowerCase();
        // for each row
        for (i = 0; i < table.rows.length; i++) {
          let row = table.rows[i];
          let compareText = row.text[table.headers[columnNumber]].toLowerCase();
          // if text is found
  ...
  function filterColumns(preFilters) {
    try {
      ...
      let table = lastTable;
      let filterButtonElem = document.getElementById(table.filterButtonId);
      ...
      if (preFilters) {
        for (let key in preFilters) {
          let headerIndex = table.headers.indexOf(key);
          if (!headerIndex && headerIndex !== 0) {
            continue;
          }
          table.filteredColumns[headerIndex] = preFilters[key];
          let filterValue = preFilters[key].toLowerCase();
          // for each row
          for (j = 0; j < table.rows.length; j++) {
            let row = table.rows[j];
            let compareText = row.text[key].toLowerCase();
            // if text is found
            if (compareText.indexOf(filterValue) !== -1) {
              // don't filter
        ...
        for (i = 0; i < table.headers.length; i++) {
          ...
          if (filterText !== "") {
            ...
            for (j = 0; j < table.rows.length; j++) {
              let row = table.rows[j];
              let compareText = row.text[table.headers[i]].toLowerCase();
              // if text is found
              if (compareText.indexOf(filterValue) !== -1) {
                // don't filter
  ...
  function updateDomFiltered(table, rebuild) {
    try {
      ...
      let captionElem = $("#" + table.captionId);
      let filterButtonElem = $("#" + table.filterButtonId);
      ...
      let divideInto = Math.ceil(rowCount / 50);
      let chunkSize = Math.ceil(rowCount / divideInto);
      let iteration = 0;
      ...
      setTimeout(function filterRows() {
        ...
        for (let i = 0; i < chunkSize; i++) {
          ...
          if (row.filteredCells) {
            for (let j = 0; j < row.filteredCells.length; j++) {
          ...
          if (row.filteredOut !== row.originalFilteredOut) {
            let id = table.title + "-row-" + row.originalOrder;
            // let elem = $("[id='"+table.title+'-row-'+originalRowNumber);
            if (row.filteredOut) {
              let elem = (document.getElementById(id).style.display = "none");
              // elem.hide();
            } else {
        ...
        if (iteration < divideInto) {
          ...
          captionElem.text(
            "Showing " +
              (table.rows.length - table.filteredCount) +
              " of " +
              table.rows.length +
              " " +
              table.caption
  ...
  function setBPTags(tags, title) {
    try {
      ...
      for (i = 0; i < table.rows.length; i++) {
        ...
        $("#row-" + row.originalOrder + "-bptags").html(
          row.tableRow[bpTagsHeader].join("<br />")
        );
      }
    } catch (error) {
      console.log(error);

  ...
  try {
    ...
    if (result.statusCode != 200) {
      if (!result.body.includes('not found')) {
        log.error('request failed with opts: ' + JSON.stringify(opts) + ', result: ' + JSON.stringify(result));
      }
      return;
    }
    var body = JSON.parse(result.body);
    if (body.error) {
      log.error('REQUEST error in makeRequest: ' + JSON.stringify(body.error) + ', opts: ' + JSON.stringify(opts));
      return;
    }
...
exports.getTransaction = async function (currency, txid) {
    ...
    if (currency === 'btc') {
      ...
      return log.error('Unrecognized currency: ' + currency + ' for txid: ' + txid);
    }

...
*/
    ...
    function traverseConfig(o, conf) {
      ...
      for (let i in o) {
        let oIsObj =
          (o[i] !== null && typeof o[i] == "object") || Array.isArray(o[i]);
        let confIsObj =
          (conf[i] !== null && typeof conf[i] == "object") ||
          Array.isArray(conf[i]);
        if (oIsObj && confIsObj) {
          // if both keys are for objects, go deeper
          traverseConfig(o[i], conf[i]);
        } else if (
          !(o[i] !== null && typeof o[i] == "object") &&
          !(conf[i] !== null && typeof conf[i] == "object")
        ) {
          // if neither key is an object, set the config property
          changes +=
            "Changed config " + i + " from " + conf[i] + " to " + o[i] + "\n";
          conf[i] = o[i];
        } else {
    ...
    log.info(
      "curl /reconfig route error with post body: " +
        JSON.stringify(body) +
        ", err: " +
        err.stack
    );
...
router.get("/getConvertedBalances", async function (ctx) {
  try {
    const results = await stats.getConvertedBalances();

...
exports.create = async function (opts, side, retries) {
  try {
    ...
    if (retries > 3) {
      return log.error(
        premsg +
          "order failed on " +
          retries +
          " attempts, orderParams: " +
          JSON.stringify(opts[side].orderParams)
      );
    }
    ...
    if (!config.gdax.realOrders) {
      return log.info(
        premsg +
          "*** STOP *** b/c config.gdax.realOrders, orderParams: " +
          JSON.stringify(opts[side].orderParams, null, 2)
      );
    }
    ...
    log.debug(
      "*** " +
        ...
        " " +
        premsg +
        JSON.stringify(_.omit(orderResult, orderResultOmit))
    );
    if (!orderResult) {
      opts[side].lastSentPrice = 0;
      return log.error(
        premsg +
          "No order result received, orderParams: " +
          JSON.stringify(opts[side].orderParams)
      );
    }
    if (
      ...
      if (orderResult.reject_reason === "post only") {
        ...
        let updatedPrice =
          side === "buy"
            ? subRound8(price, opts.increment)
            : addRound8(price, opts.increment);
        opts[side].cancelEdgePrice =
          side === "buy"
            ? subRound8(opts[side].cancelEdgePrice, opts.increment)
            : addRound8(opts[side].cancelEdgePrice, opts.increment);
        retries++;
        log.debug(
          premsg +
            "*** rejected post_only ***, RETRY best_bid: " +
            opts.product.ticker.best_bid +
            ", best_ask: " +
            opts.product.ticker.best_ask +
            ", orderParams: " +
            JSON.stringify(opts[side].orderParams)
        );
    ...
    if (orderResult.status !== "pending") {
      ...
      log.error(
        premsg +
          "Expected order to be pending, instead status = " +
          orderResult.status +
          ", orderResult: " +
...
exports.handleOrder = function (data) {
  // send new order data here
  try {
    ...
    if (!product)
      return log.error(
        productId +
          " [handleOrder] Trade data not found for data: " +
    ...
    if (side !== "buy" && side !== "sell")
      return log.error(premsg + "Unrecognized side");
    let id = getIdFromOrder(data); // data.id || data.order_id || data.maker_order_id;
    if (!id)
      return log.error(
        "Could not get order id from order data: " + JSON.stringify(data)
      );
    let order = orders[id];
    ...
    if (!order) {
      // order is new
      if (data.type && data.type !== "limit") {
        // orders with a type come from gdax
        return log.info(
          premsg +
            "Order should exist for type: " +
            data.type +
            ", data: " +
            JSON.stringify(data) +
            ", orders: " +
      ...
      log.debug(
        "data.response: " +
          JSON.stringify(_.omit(order.data.response, responseOmit))
      );
      // lower available on the account that is decreasing
      let account = side === "buy" ? quoteAccount : baseAccount;
      let changeSize =
        side === "buy" ? multRound8(order.size, order.price) : order.size;
    ...
    log.debug(
      "***  DONE " +
        ...
        " data: " +
        premsg +
        JSON.stringify(_.omit(data, doneOmit), null, 2)
    );
    if (data.reason === "filled") {
      ...
      log.debug(
        side +
          " lastFillTime updated to " +
          product[side].lastFillTime +
          ", lastFillPrice: " +
          product[side].lastFillPrice
      );
      ...
      log.error(
        product.currency +
          " Unrecognized data type reason: " +
          data.reason +
          ", data: " +
          JSON.stringify(data, null, 2)
      );
    }
    let baseChangeSize = subRound8(
      order.data.response.size,
      data.remaining_size
    );
    let baseUnchangeSize = subRound8(order.data.response.size, baseChangeSize);
    if (baseChangeSize < 0)
      return log.error(premsg + "Size changed by less than zero");
    let quoteChangeSize = multRound8(baseChangeSize, order.price);
    if (quoteChangeSize < 0)
      return log.error(
        premsg + "Quote currency size changed by less than zero"
      );
    ...
    if (!order.isCanceled && (!baseChangeSize || isNaN(baseChangeSize))) {
      log.error(
        premsg +
          "No quoteChangeSize after checking size and remaining_size, quoteChangeSize: " +
          quoteChangeSize +
          ", isAllFilled: " +
          isAllFilled +
          ", remainingSize: " +
    ...
    if (!baseChangeSize) {
      ...
      if (side === "buy") {
        ...
        quoteAccount.available = addFloor8(
          quoteAccount.available,
          quoteUnchangeSize
        );
      } else {
        // add back to available base currency
        baseAccount.available = addFloor8(
          baseAccount.available,
          baseUnchangeSize
        );
      }
    } else {
      if (side === "buy") {
        ...
        quoteAccount.balance = floor8(
          math.add(quoteAccount.balance, quoteChangeSize)
        );
        quoteAccount.available = floor8(
          math.add(quoteAccount.available, baseChangeSize)
        );
      }
      product[side].lastFillTime = new Date();
      product[side].lastFillPrice = order.price;
      ...
      log.debug(
        premsg + "lastFillPrice updated: " + JSON.stringify(product.stats)
      );
    }
    log.debug(
      premsg +
...
exports.cancelOutsideRange = async function (opts, side, edge) {
  try {
    ...
    for (let id in product.orders[side]) {
      ...
      if (
        order.pendingCancel ||
        isPastEdge(side, edge, order.price) ||
        isPastEdge(otherSide, edge2, order.price)
      ) {
        log.debug(
          premsg +
            "canceling id: " +
          ...
          if (order.price === remainingOrders[i].price) {
            ...
            if (ordDate > remDate) {
              ...
              log.debug(
                premsg +
                  "canceling id: " +
                  remainingOrders[i].id +
                  " at price: " +
                  remainingOrders[i].price
              );
              ...
              log.debug(
                premsg + "canceling id: " + id + " at price: " + order.price
              );
              await cancelById(id);
              order.pendingCancel = new Date();
              wereCanceled = true;
    ...
    if (remainingOrders.length > possible) {
      ...
      if (numToRemove < 0)
        return log.error(premsg + "numToRemove: " + numToRemove + " < 0");
...
function getIdFromOrder(ord) {
  try {
    ...
    if (!type) {
      log.error(
        "[getIdFromOrder] id not found for order: " + JSON.stringify(ord)
      );
    }
...
function getOrderSize(product, ord) {
  try {
    ...
    let orderSize =
      ...
      ord.remaining_size ||
      ord.data.size ||
      ord.data.response.size;
    if (!orderSize) {
      log.error(
        "[getOrderSize] size not found for order: " + JSON.stringify(ord)
      );
    } else {
...
function removeOrder(product, order) {
  try {
    ...
    if (!found) {
      log.error(
        product.id +
          " Could not find order to delete after sell, side: " +
          side +
          ", order: " +
          JSON.stringify(order) +
          ", product.orders[side]: " +
          JSON.stringify(product.orders[side])
      );
    }
    return sizeRemoved;
  } catch (err) {
...
function checkAwaits() {
  // don't let a missing response for an order keep awaiting
  try {
    let now = new Date();
    let sides = ["buy", "sell"];
    for (let id in config.products) {
      ...
      for (let j = 0; j < sides.length; j++) {
        ...
        if (product.awaiting[side]) {
          ...
          if (msPast < 20000) {
            ...
            log.info(
              product.currency + " checkAwaits removed await for " + side
            );
          }
        }
      }

...
exports.checkBalances = async function () {
  ...
  for (let i = 0; i < accounts.length; i++) {
    ...
    utility.parseFloatObject(account, accountFloatProperties);
    if (
      utility.floor8(exports.accounts[account.currency].balance) !==
      utility.floor8(account.balance)
    ) {
      if (account.balance - exports.accounts[account.currency].balance > 1) {
        log.error(
          account.currency + " is off by over 1\n" + JSON.stringify(account)
        );
      } else {
        log.debug(
          account.currency +
            " balance from gdax " +
            exports.accounts[account.currency].balance +
            " did not equal calculated balance " +
            utility.floor8(account.balance)
        );
      }
    ...
    if (
      utility.floor8(exports.accounts[account.currency].available) !==
      utility.floor8(account.available)
    ) {
      if (
        account.available - exports.accounts[account.currency].available >
        10
      ) {
        log.error(
          account.currency +
            " available from gdax " +
            exports.accounts[account.currency].available +
            " did not equal calculated available " +
            utility.floor8(account.available)
        );
        exports.accounts[account.currency].available = utility.floor8(
          account.available
        );
      } else {
        log.debug(

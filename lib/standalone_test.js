...
function init() {
  try {
    ...
    for (let i = 0; i < dataPointsCount; i++) {
      ...
      let price =
        Math.round((prices[prices.length - 1] + priceJump * sign) * 100) / 100;
      if (price < minPrice) {
        price = minPrice;
      }
      if (price > maxPrice) {
    ...
    for (let i = 0; i < prices.length; i++) {
      ...
      let lowHeld = held * price < 0.1 * usd;
      let isEnoughUSD = usd > 100;
      if (isEnoughUSD && (lowHeld || isGoodBuy || isPriceLow)) {
        // buy
        let toBuy;
      ...
      if (true || choice === "x") {
        console.log(
          price +
            "," +
            utility.round(currentValue, 2) +
            "," +
            utility.round(holdOnlyQty * price, 2) +
            "," +
            utility.round(held, 2) +
            "," +
            choice +
            "," +
            utility.round(usd, 2) +
            ...
            coinQty +
            "," +
            utility.round(avgPrice, 2) +
            "," +
            utility.round(avgBuyPrice, 2) +
            "," +
            lowHeld +
            "," +
            isGoodBuy +
    ...
    console.log(
      "ENDING held: " + Math.round(held) + ", usd: " + Math.round(usd)
    );
    let finalValue = held * prices[prices.length - 1] + usd;
    console.log(
      "holdOnly: " +
        Math.round(holdOnlyFinalValue) +
        ", strategy: " +
    ...
    console.log(
      "winner: " +
        winner +
        ", diff: " +
        Math.round(finalValue - holdOnlyFinalValue)
    );
    console.log(
      "bought: " + Math.round(bought) + ", sold: " + Math.round(sold)
    );
  } catch (err) {
    log.error(err.stack);
  }

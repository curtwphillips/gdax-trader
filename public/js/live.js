...
var tableData = _.extend({}, defaultTableData, reports[report].tableData);
tableData.rowData = data;
var role = auth.getUserSettings().role;
...
*/
  ...
  var tableOrder = [ // display order on live.html
    'os',
    'general',
    'btc',
    'ltc',
  ...
  var errorMsgId = 'live-error-msg';
  var fullData = {};
  var tableMade = {};
  var hostOrder = []; // fill with hostnames
  var notReady = true; // init hasn't run
  var cbTracker = {}; // keep track of checkboxes
  var userSettings = {};
  ...
  var defaultTableData = {
    ...
    headerConversions: { // replace key with value in headers list
      'Tags': 'live Tags',
      'dayOfWeek': 'Days',
      Bytes: 'GB'
    },
  ...
  function cbChanged (dataOwner) {
    var cb = document.getElementById('live-'+dataOwner+'-checkbox');
    if (cb && cbTracker[dataOwner] && !cb.checked) { // unchecked
...
var dataEdit;
      ...
      var freezeCheckbox = document.getElementById('live-frozen-checkbox');
      if (freezeCheckbox && freezeCheckbox.checked) return; // updating turned off
      var rowData;
      var make = []; // holds tableNames that do not exist yet
      for (var tableName in data) { // make key available for comparison
        var tableExists = document.getElementById('live-' + tableName + '-table');
        if (!tableExists) make.push(tableName);
        var rowExists = false;
        for (var k in data[tableName]) {
          // data[tableName][k] = tables.replaceChars(data[tableName][k], errorMsgId);
          data[tableName][k] = JSON.stringify(data[tableName][k]);
          if (tableExists) {
            var colElem = $('#live-'+tableName+'-table th[header-text="' + k + '"]');
            var headerText = colElem.attr('header-text');
            if (!headerText) { // create the column
              if (k !== 'hostname') {
                tables.buildCol('live-' + tableName, k);
              }
              
            }
            var rowElem = $('#live-'+tableName+'-table-tbody tr[hostname="' + dataOwner + '"]');
            // var rowElem = $('#live-'+tableName+'-table-tbody');
            if (rowElem) {
              var rowNum = rowElem.attr('row-num');
              if (rowNum) {
                rowExists = true;
                var elem = $('#live-'+tableName+'-table-tbody #row-'+rowNum+'-'+k);
                if (elem) {
                  var elemText = elem.text();
                  var updateText = data[tableName][k]; // update value in table
        ...
        if (tableExists && !rowExists) {
          ...
          tables.buildRow('live-' + tableName, rowNum, {pristineRow: data[tableName]}, null, dataOwner);
        }
      }
      if (make.length > 0) {
        var orderedTables = [];
        for (var i = 0; i < tableOrder.length; i++) { // get ordered columns
          var index = make.indexOf(tableOrder[i]);
          if (index !== -1) {
            orderedTables.push(tableOrder[i]); // add column to ordered list
            make.splice(index, 1); // remove the column from original list
          }
        }
        for (var i = 0; i < make.length; i++) { // get unordered columns
          orderedTables.push(make[i]);
        }
        for (var i = 0; i < orderedTables.length; i++) { // make tables
  ...
  function makeTable(tableName, data, hostname, rowData) {
    try {
      ...
      var appendElem = document.getElementById('live-tables-container');
      var rowData = rowData || [data[tableName]];
      ...
      if (!$('#live-'+tableName+'-table-container').length) { // build the base html if not exists

...
exports.logRequest = async function (ctx, next) {
  ...
  if (Object.keys(ctx.request.body).length !== 0 && ctx.request.body.constructor === Object) {
    var bod = _.omit(ctx.request.body, 'password');
    if (ctx.request.body.password) {
      bod.password = '<redacted>';
    }
    log.debug(ctx.method+' '+ctx.url+' body: '+JSON.stringify(bod));
  }
  if (Object.keys(ctx.request.query).length !== 0 && ctx.request.query.constructor === Object) {
    var qbod = _.omit(ctx.request.query, 'password');

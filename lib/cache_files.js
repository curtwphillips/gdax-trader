  ...
  try {
    ...
    try {
      ...
      if (!fileName.startsWith("backup_")) {
        log.info(
          "--- Checking backup exchange cache file " + fileName + " ---"
        );
      ...
      if (err.code !== "ENOENT") {
        ...
        log.info(
          err.stack ||
            "There was an error getting " + fileName + ", error: " + err
        );
      } else {
...
exports.writeFile = async function (fileName, data) {
  try {
    ...
    if (!fileName) {
      throw new Error(
        "Write to cache file missing file name, data: " + JSON.stringify(data)
      );
    }
    ...
    const text = getFileText(data);

    // delete the text of the current file
    await utility.truncateFile(__dirname + "/../cache/" + fileName, 0);

    const source = __dirname + "/../cache/" + fileName;
    const target = __dirname + "/../cache/backup_" + fileName;

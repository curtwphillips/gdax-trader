...
async function shutDown () {
  setTimeout(function () { // ensure exit within a few seconds if await takes a while
    exitProcess();
  }, 2000);
  ...
  if (listeners.length) {
    ...
    listeners.forEach(function (listener) {
      ...
      setImmediate(function(){listener.emit('close')});
    });
  }
...
// catch unhandled promises - out of use for now because it made error tracking harder
// process.on('unhandledRejection', function (err) {
//   log.error('unhandledRejection: ' + JSON.stringify(err));
// });

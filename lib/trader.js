 ...
 */
exports.onPrice = async function (productId, ticker) {
  // LTC-USD 60.51, LTC-BTC .0085
  try {
    let opts;
    if (config.server.shuttingDown) return;
    const premsg = "[onPrice] " + productId;
    ...
    if (config.gdax.delayPrice && product.lastOrderStarted) {
      ...
      log.debug(
        premsg + " price delay is set to " + config.gdax.delayPrice + "ms"
      );
    }
...
async function setRanges(opts, side) {
  try {
    let premsg =
      ...
      side +
      ", $" +
      opts.product.ticker.price +
      ", ";
    if (!opts.product.config[side].on) return;
    log.debug(
      "*** " + opts.trackingNumber + " START SET RANGES " + side.toUpperCase()
    );
    ...
    log.debug(
      side +
        " opts[side].tooHighPrice: " +
        opts[side].tooHighPrice +
        ", opts[side].tooLowPrice: " +
        opts[side].tooLowPrice +
        ", opts.product[side].lastFillPrice: " +
        opts.product[side].lastFillPrice +
        ", opts.product[otherSide].lastFillPrice: " +
        opts.product[otherSide].lastFillPrice +
        ", opts.product[side].lastFillTime: " +
        opts.product[side].lastFillTime +
        ", opts.product[otherSide].lastFillTime: " +
        JSON.stringify(opts.product[otherSide].lastFillTime)
    );
    // ensure qty < max
    if (opts[side].maxSize && opts[side].useSize > opts[side].maxSize)
      opts[side].useSize = opts[side].maxSize;
    // ensure qty > min
    if (opts[side].minSize && opts[side].useSize < opts[side].minSize)
      opts[side].useSize = opts[side].minSize;
    // ensure price < max
    if (opts[side].maxPrice && opts[side].tooHighPrice > opts[side].maxPrice)
      opts[side].tooHighPrice = opts[side].maxPrice;
    // ensure price > min
    if (opts[side].minPrice && opts[side].tooLowPrice < opts[side].minPrice)
      opts[side].tooLowPrice = opts[side].minPrice;
    ...
    if (side === "buy") {
      ...
      opts[side].basePrice = utility.nearestMod(
        opts[side].tooHighPrice,
        opts.increment
      );
      ...
      if (opts.product.quoteAccount.config.minBalance) {
        opts[side].minBalance = divFloor8(
          opts.product.quoteAccount.config.minBalance,
          opts[side].basePrice
        );
      } else {
      ...
      log.debug(
        side +
          " minBalance: " +
          opts[side].minBalance +
          ", opts.product.quoteAccount.balance: " +
          opts.product.quoteAccount.balance +
          ", opts[side].basePrice: " +
          opts[side].basePrice
      );
      opts[side].balance = divFloor8(
        opts.product.quoteAccount.balance,
        opts[side].basePrice
      );
    ...
    if (opts[side].useSize > opts[side].available) {
      if (opts[side].available >= opts[side].minSize) {
        ...
        return log.info(
          premsg +
            "STOP STOP STOP STOP STOP, balance is too low, balance: " +
            opts[side].balance +
            ", available: " +
            opts[side].available +
            ", minBalance: " +
      ...
      opts[side].boostedSize &&
      opts[side].boostedSize > opts[side].useSize
    ) {
      ...
      if (opts.maxSize && opts[side].useSize > opts.maxSize)
        opts[side].useSize = opts.maxSize;
    ...
    opts[side].possible = Math.floor(
      math.divide(opts[side].available, opts[side].useSize)
    );
    ...
    if (side === "buy") {
      opts[side].cancelEdgePrice = subRound8(
        opts[side].basePrice,
        math.multiply(opts.increment, math.subtract(opts[side].possible, 1))
      );
      opts[side].cancelEdgePrice = utility.toFixedTrunc(
        opts[side].cancelEdgePrice,
        opts.precision
      );
    } else {
      opts[side].cancelEdgePrice = addRound8(
        opts[side].basePrice,
        math.multiply(opts.increment, math.subtract(opts[side].possible, 1))
      );
      opts[side].cancelEdgePrice = utility.toFixedTrunc(
    ...
    for (let i = 0; i < opts[side].possible; i++) {
      if (side === "buy") {
        opts[side].incrementedPrice = subRound8(
          opts[side].basePrice,
          math.multiply(opts.increment, i)
        );
      } else {
        // side === 'sell'
        opts[side].incrementedPrice = addRound8(
          opts[side].basePrice,
          math.multiply(opts.increment, i)
        );
      }
      opts[side].incrementedPrice = math.round(
        opts[side].incrementedPrice,
        opts.precision
      );
      ...
      opts[side].ordersSizeSum = addRound8(
        opts[side].ordersSizeSum,
        opts[side].useSize
      );
    }
    // cancel orders out of range
    // log.debug(premsg + 'cancelEdgePrice: ' + opts.cancelEdgePrice + ', possible: ' + opts.possible);
    let wereCanceled = await orders.cancelOutsideRange(
      opts,
      side,
      opts[side].cancelEdgePrice
    );
    ...
    log.debug(
      "*** " + opts.trackingNumber + " FINALLY SET RANGES " + side.toUpperCase()
    );
  }
}
...
function getBaseUnheldBalance(opts) {
  return subFloor8(
    opts.product.baseAccount.balance,
    opts.product.baseAccount.config.minBalance || 0
  );
}
...
function getRateFromPercent(opts, side, percent) {
  ...
  return addRound8(
    ...
    math.divide(
      math.multiply(
        math.subtract(percent, lowPercent),
        math.subtract(highRate, lowRate)
      ),
      math.subtract(highPercent, lowPercent)
    )
  );
}
...
function setTooHighPrice(opts, side) {
  try {
    ...
    let holdQuotePercent = getRateHoldPercent(
      opts,
      side,
      opts[side].tooHighPrice
    );
    ...
    if (heldNeeded > quoteUnheldBalanceConverted) {
      ...
      opts[side].tooHighPrice = Math.min(
        opts[side].tooHighPrice,
        getRateFromPercent(
          opts,
          side,
          divRound8(quoteUnheldBalanceConverted, fullHeld)
        )
      );
    }
    log.debug(
...
function getUseSize(opts) {
  try {
    ...
    let balance = Math.max(
      utility.subFloor8(
        utility.floor8(opts.product.baseAccount.balance),
        opts.product.baseAccount.config.minBalance
      ),
      0
    );
    log.debug(
      "getUseSize " +
        opts.product.baseAccount.currency +
        " unheld balance: " +
        balance +
        ", full " +
        opts.product.baseAccount.currency +
...
function getQuoteCurrencyConvertedUsableBalance(opts) {
  ...
  if (minOtherBalance) {
    otherBalance = Math.max(
      utility.subFloor8(otherBalance, minOtherBalance),
      0
    );
  }
